...
OUTPUT_CSV="${OUTPUT_CSV:-$DEMO_DIR/assets/compression-stats.csv}"
...
run_hyperfine_scan() {
  ...
  echo "Benchmarking $algo (levels $min..$max)..."
  hyperfine \
...
measure_compressed_bytes() {
  ...
  case "$algo" in
    gzip)
      bytes="$(gzip "-$level" -c "$INPUT_FILE" | wc -c | tr -d ' ')"
      ;;
    brotli)
      bytes="$(brotli -q "$level" -c "$INPUT_FILE" | wc -c | tr -d ' ')"
      ;;
    zstd)
      bytes="$(zstd "-$level" -c "$INPUT_FILE" | wc -c | tr -d ' ')"
      ;;
    dcb)
      bytes="$(brotli -q "$level" -D "$DCB_DICT" -c "$INPUT_FILE" | wc -c | tr -d ' ')"
      ;;
    dcz)
      bytes="$(zstd "-$level" -D "$DCZ_DICT" -c "$INPUT_FILE" | wc -c | tr -d ' ')"
      ;;
    *)
